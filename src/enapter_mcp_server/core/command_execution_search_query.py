...
class CommandExecutionSearchQuery:
    ...
    command_name_pattern: str | None = None
    state: domain.CommandExecutionState | None = None

    @functools.cached_property
    def _command_name_re(self) -> re.Pattern[str] | None:
        if self.command_name_pattern is None:
            return None
        return re.compile(self.command_name_pattern)
    ...
    def matches(self, execution: domain.CommandExecution) -> bool:
        ...
        if self.state is not None and execution.state != self.state:
            return False
        if self._command_name_re is not None and not self._command_name_re.search(
